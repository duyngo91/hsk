...
│   │   └── system-a/
│   │       └── login.page.ts
│   ├── screens/                    # Quản lý Mobile Screen Object Model (POM)
│   │   └── system-a/
│   │       └── login.screen.ts
│   └── fixtures/                   # Nơi định nghĩa Dependency Injection (DI)
│       └── base.fixture.ts         # Tiêm tự động webApp, mobileApp, apiClient, dbClient vào spec test
├── tests/                          # Nơi lưu trữ các Test Specs viết bằng TypeScript
│   └── system-a/
│       └── feature-1/
│           ├── api-user.spec.ts    # Test Case kiểm thử API
│           ├── web-login.spec.ts   # Test Case kiểm thử Web
│           └── hybrid-e2e.spec.ts  # Test Case tích hợp chéo (Web + Mobile + DB + API)
...
Bạn có thể dễ dàng chạy các bộ kiểm thử tương ứng trên từng môi trường khác nhau bằng các NPM Scripts có sẵn:
...
```bash
npm run test:dev -- tests/system-a/feature-1/hybrid-e2e.spec.ts
```
